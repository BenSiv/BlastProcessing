    ...
    for blast in eachrow(Blast)
        ...
        for record in Genome
            if FASTA.identifier(record) == blast.Reference
                if blast.Strand == "+"
                    if blast.Ref_Start-200 < 1
                        push!(Promoters, FASTA.sequence(record)[1:blast.Ref_Start+1000])    
                    else
                        push!(Promoters, FASTA.sequence(record)[blast.Ref_Start-200:blast.Ref_Start+1000]) 
                    end
                else
                    if blast.Ref_End+200 > length(FASTA.sequence(record))
                        push!(Promoters, FASTA.sequence(record)[blast.Ref_End-1000:end]) 
                    else
                        push!(Promoters, FASTA.sequence(record)[blast.Ref_End-1000:blast.Ref_End+200]) 
                    end
                end
            end
        end
